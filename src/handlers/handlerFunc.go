...
func GetLastUpdateAPKFunc(ctx *gin.Context) {
	...
	configFile, err := os.Open(dir + config.AppConfig.SettingsParseUpdateAPKFile.PathFile)
	if err != nil {
		resp.Error = err.Error()
		RespondWithMessage(http.StatusOK, 0, resp, ctx)
		return
...
func DownloadLastUpdateAPKFunc(ctx *gin.Context) {
	...
	fullNamePath := dir + config.AppConfig.SettingsParseUpdateAPKFile.Path + "/" + autoGenerate[0].ApkInfo.OutputFile
